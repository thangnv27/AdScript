...
class OffersController extends AppController {
     ...
     * Giao dịch mua/bán
     */
    public function trade($id = 0){
        $action = 'buy';
        $title = 'Giao dịch Mua WEX (USD)';
        $meta_description = "";
        $og_type = "article";
        $og_url = SITE_URL . Router::url(['controller' => 'Offers', 'action' => 'trade']);
        $og_image = SITE_URL . '/img/frontend/logo.png';
        $message = "";
        $success = "";
        
        $offer = $this->Offers->find('all', array(
                'conditions' => array(
                    'Offers.id' => $id,
                )
            ))->contain(['Users'])->first();
        if($offer){
            ...
            $offer_user = $this->Offers->Users->find('all', array(
                'conditions' => array(
                    'Users.id' => $offer->user_id,
                )
            ))->contain(['Wallets', 'UserMeta'])->first();
            
            if($offer->offer_type == "buy"){ // Bán WEX
                $action = "sell";
                $title = 'Giao dịch Bán WEX (USD)';
                
                if($curent_user->id == $offer->user_id){
                    $message = __('Bạn không thể tự giao dịch với chính mình!');
                } else if($curent_user->wallet->usd < $offer->min_amount){
                    $message = __('Bạn không có đủ số dư cho giao dịch này!');
                }
            } else { // Mua WEX
                if($curent_user->id == $offer->user_id){
                    $message = __('Bạn không thể tự giao dịch với chính mình!');
                } else if($offer_user->wallet->usd < $offer->min_amount){
